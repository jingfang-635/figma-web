...
description: >-
  ...
  Screen Blueprint → codegen；默认 antd 高还原前端 + 双闸门（字段一致性 + SSIM）+
...
- **默认启用**高还原前端方案（antd + Layout IR + Blueprint + 双闸门），还原度不是可选项。
...
2. **后端语言**：Node（默认）/ Java
3. **栈组合**：默认 A（React + NestJS + Prisma）；见 reference.md 矩阵
4. **页面范围**：已画屏 / 导航全做
5. **鉴权**：JWT（默认）/ 无
...
npm run visual:gen      # tokens.css / antdTheme / blueprints / screenConfigs
...
3. 顺序：init-project → Spec 闸门 → 视觉抽取 → 字段回填 → visual:gen → DB/API → antd 前端 → 双闸门（第一版）→ **还原轮次**
